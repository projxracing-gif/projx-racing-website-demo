...
- ECS Tuning: ECS has approved automated product copying and advised that a private FTP catalogue export may be possible, but access, credentials, fields and refresh cadence are still pending. The public sitemap index does not make its Cloudflare-challenged product shards an approved ingestion route. Use bounded scheduled checks only for explicitly listed public SKUs while the private authorization gate is valid. Do not publish FTP dealer cost, bypass access controls or label observations as live stock. Match any future update schedule to the supplier's actual FTP refresh cadence.
