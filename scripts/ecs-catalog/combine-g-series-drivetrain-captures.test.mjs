...
import {
  combineGSeriesDrivetrainCaptures,
  DRIVETRAIN_CAPTURE_VEHICLES
} from './combine-g-series-drivetrain-captures.mjs';
...
const manifest = {
  ...
  counts: {
    ...
    'BMW G80 M3 Competition S58 3.0L': { 'fixture-components': 17, 'fixture-tools': 0 },
    'BMW G82 M4 Competition S58 3.0L': { 'fixture-components': 1, 'fixture-tools': 1 }
  }
};

const quarantinedManifest = structuredClone(manifest);
quarantinedManifest.categories['fixture-pdk'] = {
...
function captureFor(vehicle, vehicleIndex, scopeManifest = manifest) {
  ...
  Object.entries(scopeManifest.categories).forEach(([key, category], categoryIndex) => {
    ...
    for (let position = 1; position <= count; position += 1) {
      const digits = position === 1
        ? '4700001'
        : `48${vehicleIndex}${categoryIndex}${String(position).padStart(4, '0')}`;
      records.push({
        ...
        productUrl: digits === '4700001'
          ? 'https://www.ecstuning.com/b-fixture-parts/shared-drivetrain-item/shared-drivetrain/'
  ...
  return {
    schemaVersion: 1,
    supplier: 'ECS Tuning',
    kind: `${vehicle.key}-drivetrain-listing-capture`,
...
function fixtureCaptures(scopeManifest = manifest) {
  return Object.fromEntries(DRIVETRAIN_CAPTURE_VEHICLES.map((vehicle, index) => [
    vehicle.key,
    captureFor(vehicle, index, scopeManifest)
  ]));
}

test('strictly combines complete keyed Drivetrain captures, including the Tools route', () => {
  const { combined, summary } = combineGSeriesDrivetrainCaptures(fixtureCaptures(), manifest);
  assert.equal(combined.schemaVersion, 1);
  assert.equal(combined.supplier, 'ECS Tuning');
  assert.equal(combined.kind, 'g-series-drivetrain-listing-capture');
  ...
  assert.equal(combined.vehicleCategories[1].categories[1].url,
    'https://www.ecstuning.com/BMW-G82-M4_Competition-S58_3.0L/Drivetrain/Tools/');
  assert.equal(combined.scopeManifest.categories['fixture-tools'].sourcePath, 'Drivetrain/Tools/');
  assert.equal(combined.scopeManifest.counts['BMW G80 M3 Competition S58 3.0L']['fixture-components'], 17);
...
test('preserves quarantined audit rows and counts while identifying the customer-facing exclusion', () => {
  const { combined, summary } = combineGSeriesDrivetrainCaptures(
    fixtureCaptures(quarantinedManifest), quarantinedManifest
  );
  assert.equal(combined.records.length, 21);
  assert.equal(combined.scopeManifest.categories['fixture-pdk'].excludeFromCustomerFacing, true);
  assert.equal(combined.quarantinedCategories.length, 1);
  assert.equal(combined.quarantinedCategories[0].slug, 'drivetrain-pdk-transmission');
  const quarantinedRecord = combined.records.find(record => record.category === 'Fixture PDK Transmission Parts');
  assert.equal(quarantinedRecord.excludeFromCustomerFacing, true);
  assert.equal(quarantinedRecord.catalogueDisposition, 'quarantined');
  const quarantinedVehicleCategory = combined.vehicleCategories
    .find(entry => entry.vehicle === 'BMW G82 M4 Competition S58 3.0L').categories
    .find(category => category.name === 'Fixture PDK Transmission Parts');
  assert.equal(quarantinedVehicleCategory.excludeFromCustomerFacing, true);
  ...
  assert.equal(summary.customerFacingRawRecordCount, 16,
    'Every placement of an identity observed in a quarantined category must be excluded downstream.');

  const missingRecordQuarantine = fixtureCaptures(quarantinedManifest);
  const record = missingRecordQuarantine.g82.records
    .find(item => item.category === 'Fixture PDK Transmission Parts');
  delete record.excludeFromCustomerFacing;
  assert.throws(() => combineGSeriesDrivetrainCaptures(missingRecordQuarantine, quarantinedManifest),
    /invalid or unreconciled/);
});

test('rejects incomplete, misordered or malformed capture category manifests', () => {
  const wrongSupplier = fixtureCaptures();
  wrongSupplier.g80.supplier = 'Other';
  assert.throws(() => combineGSeriesDrivetrainCaptures(wrongSupplier, manifest), /capture is incomplete or invalid/);

  const missingCategory = fixtureCaptures();
  missingCategory.g82.categories.pop();
  assert.throws(() => combineGSeriesDrivetrainCaptures(missingCategory, manifest), /category manifest is incomplete/);

  const wrongPagination = fixtureCaptures();
  wrongPagination.g80.categories[0].pageCounts = [15, 2];
  assert.throws(() => combineGSeriesDrivetrainCaptures(wrongPagination, manifest), /category manifest is invalid/);
});

test('recomputes record positions and enforces exact full vehicle-relative category routes', () => {
  const wrongToolsRoute = fixtureCaptures();
  const toolsRecord = wrongToolsRoute.g82.records.find(record => record.category === 'Fixture Drivetrain Tools');
  toolsRecord.sourceUrl = 'https://www.ecstuning.com/BMW-G82-M4_Competition-S58_3.0L/Tools/Drivetrain/';
  assert.throws(() => combineGSeriesDrivetrainCaptures(wrongToolsRoute, manifest), /invalid or unreconciled/);
  ...
  lastRecord.sourceUrl = 'https://www.ecstuning.com/BMW-G80-M3_Competition-S58_3.0L/Drivetrain/Fixture_Components/';
  assert.throws(() => combineGSeriesDrivetrainCaptures(lyingPositionsFlag, manifest), /positions do not reconcile/);
});

test('rejects conflicting supplier identity and non-namespaced manifest slugs', () => {
  const identityConflict = fixtureCaptures();
  identityConflict.g82.records[0].manufacturerPartNumber = 'CONFLICTING-MPN';
  assert.throws(() => combineGSeriesDrivetrainCaptures(identityConflict, manifest), /Conflicting supplier identity/);

  const unsafeManifest = structuredClone(manifest);
  unsafeManifest.categories['fixture-tools'].slug = 'tools';
  assert.throws(() => combineGSeriesDrivetrainCaptures(fixtureCaptures(), unsafeManifest),
