...
import { createGSeriesEngineScope } from './prepare-g-series-performance.mjs';

const ECS_HOST = 'www.ecstuning.com';
const PRODUCT_PATH = /^\/b-[^/?#]+-parts\/[^/?#]+\/[^/?#]+\/$/i;
const PAGE_SIZE = 16;

export const ENGINE_CAPTURE_VEHICLES = Object.freeze([
  Object.freeze({
    key: 'g80',
...
async function firstExistingPath(filenames) {
  ...
  throw new Error(`Engine capture file not found. Checked: ${filenames.join(', ')}`);
}
...
function normalizeEngineCapture(document, vehicle, scope) {
  if (!document || !Array.isArray(document.categories) || !Array.isArray(document.records)) return document;
  const canonicalEntries = Object.entries(scope.categoryKeys).map(([key, name]) => ({
    key,
    name,
    root: categoryPageRoot(vehicle, scope, name)
  }));
  const canonicalByRoot = new Map(canonicalEntries.map(entry => [entry.root, entry]));
  const canonicalByKey = new Map(canonicalEntries.map(entry => [entry.key, entry]));
  ...
  for (const category of document.categories) {
    const pageUrls = Array.isArray(category?.pageUrls)
      ? category.pageUrls.map(url => canonicalEcsUrl(url)).filter(Boolean)
      : [];
    const sourceRoot = pageUrls[0] || canonicalEcsUrl(category?.sourceUrl);
    const canonical = canonicalByRoot.get(sourceRoot)
      || canonicalByKey.get(String(category?.key ?? '').trim())
      || canonicalByName.get(String(category?.name ?? '').trim());
    if (!canonical || normalizedByKey.has(canonical.key)) {
      throw new Error(`The ${vehicle.key.toUpperCase()} Engine category route cannot be reconciled.`);
    }
    canonicalByAlias.set(String(category?.name ?? '').trim(), canonical);
    normalizedByKey.set(canonical.key, {
      ...category,
      key: canonical.key,
      name: canonical.name,
      pageUrls
  ...
  if (normalizedByKey.size !== canonicalEntries.length) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Engine category manifest is incomplete.`);
  }
  const records = document.records.map(record => {
    const canonical = canonicalByAlias.get(String(record?.category ?? '').trim());
    if (!canonical) {
      throw new Error(`The ${vehicle.key.toUpperCase()} Engine record category cannot be reconciled.`);
    }
    return { ...record, category: canonical.name, categoryKey: canonical.key };
  });
  return {
    ...document,
    categories: canonicalEntries.map(entry => normalizedByKey.get(entry.key)),
    records
  };
}
...
function validateCategoryManifest(document, vehicle, scope) {
  ...
  return document.categories.map((category, index) => {
    ...
    if (key !== expectedKey || name !== expectedName || seenNames.has(name)
      ...
      throw new Error(`The ${vehicle.key.toUpperCase()} Engine category manifest is invalid for ${name || key || 'an entry'}.`);
    }
    seenNames.add(name);
    return {
...
function validateRecord(record, vehicle, scope, categories, index) {
  ...
  if (record?.vehicle !== vehicle.name || !declaredCategory || !digits || !productUrl
    ...
    || !quarantineMatches) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Engine record ${index + 1} is invalid or unreconciled.`);
  }
...
function validateCapture(document, vehicle, scope) {
  if (document?.schemaVersion !== 1 || document?.supplier !== 'ECS Tuning'
    || document?.kind !== `${vehicle.key}-engine-listing-capture`
    || !exactTimestamp(document?.generatedAt) || !Array.isArray(document?.records)) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Engine capture is incomplete or invalid.`);
  }
  ...
  if (document.records.length !== expectedRecordCount) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Engine record count does not reconcile with its manifest.`);
  }
  ...
  const validatedRecords = document.records.map((record, index) => {
    ...
    if (placements.has(placement)) throw new Error(`Duplicate Engine placement detected for ${placement}.`);
  ...
  for (const category of categories) {
    ...
    if (positions.length !== category.count
      ...
      throw new Error(`The ${vehicle.key.toUpperCase()} Engine positions do not reconcile for ${category.name}.`);
    }
  }
  return categories;
}
...
export function combineGSeriesEngineCaptures(capturesByVehicle, manifestDocument) {
  const scope = createGSeriesEngineScope(manifestDocument);
  const captures = ENGINE_CAPTURE_VEHICLES.map(vehicle => {
    const capture = normalizeEngineCapture(capturesByVehicle?.[vehicle.key], vehicle, scope);
  ...
  if (!generatedAt) throw new Error('The Engine capture has no valid observation timestamp.');
  const vehicleCategories = captures.map((document, index) => ({
    vehicle: ENGINE_CAPTURE_VEHICLES[index].name,
  ...
  const combined = {
    schemaVersion: 1,
    supplier: 'ECS Tuning',
    kind: 'g-series-engine-listing-capture',
    generatedAt,
  ...
  const summary = {
    ...
    vehicleCounts: Object.fromEntries(ENGINE_CAPTURE_VEHICLES.map(vehicle => [vehicle.key,
      records.filter(record => record.vehicle === vehicle.name).length])),
    nonEmptyCategoryCounts: Object.fromEntries(ENGINE_CAPTURE_VEHICLES.map((vehicle, index) => [vehicle.key,
...
async function main() {
  ...
  if (!captureDirectory || !manifestPath || !output) {
    throw new Error('Usage: combine-g-series-engine-captures.mjs --capture-dir <directory> --manifest <scope-manifest.json> --output <capture.json>');
  }
  const directory = path.resolve(captureDirectory);
  const capturePaths = await Promise.all(ENGINE_CAPTURE_VEHICLES.map(vehicle => firstExistingPath([
    path.join(directory, `${vehicle.key}-records.json`),
    path.join(directory, vehicle.key, `${vehicle.key}-records.json`)
  ])));
  const [manifestDocument, ...captures] = await Promise.all([
    readJson(path.resolve(manifestPath)),
    ...capturePaths.map(readJson)
  ]);
  const capturesByVehicle = Object.fromEntries(ENGINE_CAPTURE_VEHICLES.map((vehicle, index) => [
    vehicle.key,
    captures[index]
  ]));
  const { combined, summary } = combineGSeriesEngineCaptures(capturesByVehicle, manifestDocument);
