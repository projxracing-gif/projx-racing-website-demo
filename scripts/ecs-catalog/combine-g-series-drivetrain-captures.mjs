...
import { createGSeriesDrivetrainScope } from './prepare-g-series-performance.mjs';

const ECS_HOST = 'www.ecstuning.com';
const PRODUCT_PATH = /^\/b-[^/?#]+-parts\/[^/?#]+\/[^/?#]+\/$/i;
const PAGE_SIZE = 16;

export const DRIVETRAIN_CAPTURE_VEHICLES = Object.freeze([
  Object.freeze({
    key: 'g80',
...
function validateCategoryManifest(document, vehicle, scope) {
  ...
  if (!Array.isArray(document.categories) || document.categories.length !== expectedEntries.length) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Drivetrain category manifest is incomplete.`);
  }
  ...
  return document.categories.map((category, index) => {
    ...
    if (key !== expectedKey || name !== expectedName || seenNames.has(name)
      ...
      throw new Error(`The ${vehicle.key.toUpperCase()} Drivetrain category manifest is invalid for ${name || key || 'an entry'}.`);
    }
    seenNames.add(name);
    return {
...
function validateRecord(record, vehicle, scope, categories, index) {
  ...
  if (record?.vehicle !== vehicle.name || !declaredCategory || !digits || !productUrl
    ...
    || !quarantineMatches) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Drivetrain record ${index + 1} is invalid or unreconciled.`);
  }
...
function validateCapture(document, vehicle, scope) {
  if (document?.schemaVersion !== 1 || document?.supplier !== 'ECS Tuning'
    || document?.kind !== `${vehicle.key}-drivetrain-listing-capture`
    || !exactTimestamp(document?.generatedAt) || !Array.isArray(document?.records)) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Drivetrain capture is incomplete or invalid.`);
  }
  ...
  if (document.records.length !== expectedRecordCount) {
    throw new Error(`The ${vehicle.key.toUpperCase()} Drivetrain record count does not reconcile with its manifest.`);
  }
  ...
  const validatedRecords = document.records.map((record, index) => {
    ...
    if (placements.has(placement)) throw new Error(`Duplicate Drivetrain placement detected for ${placement}.`);
  ...
  for (const category of categories) {
    ...
    if (positions.length !== category.count
      ...
      throw new Error(`The ${vehicle.key.toUpperCase()} Drivetrain positions do not reconcile for ${category.name}.`);
    }
  }
  return categories;
}
...
export function combineGSeriesDrivetrainCaptures(capturesByVehicle, manifestDocument) {
  const scope = createGSeriesDrivetrainScope(manifestDocument);
  const captures = DRIVETRAIN_CAPTURE_VEHICLES.map(vehicle => {
  ...
  if (!generatedAt) throw new Error('The Drivetrain capture has no valid observation timestamp.');
  const vehicleCategories = captures.map((document, index) => ({
    vehicle: DRIVETRAIN_CAPTURE_VEHICLES[index].name,
  ...
  const combined = {
    schemaVersion: 1,
    supplier: 'ECS Tuning',
    kind: 'g-series-drivetrain-listing-capture',
    generatedAt,
  ...
  const summary = {
    ...
    vehicleCounts: Object.fromEntries(DRIVETRAIN_CAPTURE_VEHICLES.map(vehicle => [vehicle.key,
      records.filter(record => record.vehicle === vehicle.name).length])),
    nonEmptyCategoryCounts: Object.fromEntries(DRIVETRAIN_CAPTURE_VEHICLES.map((vehicle, index) => [vehicle.key,
...
async function main() {
  ...
  if (!captureDirectory || !manifestPath || !output) {
    throw new Error('Usage: combine-g-series-drivetrain-captures.mjs --capture-dir <directory> --manifest <scope-manifest.json> --output <capture.json>');
  }
  ...
  const [manifestDocument, ...captures] = await Promise.all([
    ...
    ...DRIVETRAIN_CAPTURE_VEHICLES.map(vehicle => readJson(path.join(directory, `${vehicle.key}-records.json`)))
  ]);
  const capturesByVehicle = Object.fromEntries(DRIVETRAIN_CAPTURE_VEHICLES.map((vehicle, index) => [
    vehicle.key,
    captures[index]
  ]));
  const { combined, summary } = combineGSeriesDrivetrainCaptures(capturesByVehicle, manifestDocument);
